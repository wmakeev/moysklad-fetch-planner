...
Планировщик, чтобы уложиться в доступные лимиты полученные из заголовков ответа сервера (напр. `X-RateLimit-Limit` и `X-RateLimit-Remaining`), автоматически вносит задержку между выполнением отдельных запросов, а так же регулирует максимальное кол-во параллельных запросов. Прозрачно выполняет повтор запроса если ошибки избежать не удалось.

Такой подход позволяет обеспечить быстрое выполнение запросов с минимальной задержкой в моменте и стабильную работу при длительной высокой нагрузке, в том числе, при наличии задач в других процессах, разделяющих общий лимит с вашим приложением.
...
- [Конфигурация](#конфигурация)
  - [`eventHandler` - обработчик событий](#eventhandler---обработчик-событий)
  - [`throttlingCoefficient` - коэффициент тротлинга](#throttlingcoefficient---коэффициент-тротлинга)
    - [Рекомендации](#рекомендации)
  - [`maxParallelLimit` - максимальное кол-во параллельных запросов](#maxparallellimit---максимальное-кол-во-параллельных-запросов)
  - [`maxRequestDelayTimeMs` - максимальное время задержки между запросами](#maxrequestdelaytimems---максимальное-время-задержки-между-запросами)
  - [`maxRequestDelayTimeJitter`](#maxrequestdelaytimejitter)
  - [`parallelLimitCorrectionPeriodMs` - период пересмотра параллельных запросов](#parallellimitcorrectionperiodms---период-пересмотра-параллельных-запросов)
...
Задержка динамически меняется в зависимости от размера доступного лимита. Если лимит близок к 100%, то задержка минимальна и начинает расти с уменьшением лимита, достигая [максимального значения](#maxrequestdelaytimems---максимальное-время-задержки-между-запросами) при 0% лимита.
...
Через [определенное время](#parallellimitcorrectionperiodms---период-пересмотра-параллельных-запросов) происходит пересмотр значения максимального значения параллельных запросов. И если за это время ошибка не повторялась, то значение снова увеличивается на `1`, постепенно восстанавливаясь до [максимума](#maxparallellimit---максимальное-кол-во-параллельных-запросов).
...
### `throttlingCoefficient` - коэффициент тротлинга
...
При минимальном значении коэффициента `throttlingCoefficient` равном `1` задержка нарастает линейно от `0` мс при 100% лимита до `maxRequestDelayTimeMs` мс при 0% лимита. При значении коэффициента больше `1`, задержка начинает нарастать нелинейно, уменьшая задержку при лимитах близких к 100% и увеличивая при уменьшении лимита. Например если коэффициент равен `10`, то тротлинга практически не будет вплоть до ~20% от лимита, а далее начнется резкое увеличение задержки до `maxRequestDelayTimeMs`.
...
### `maxParallelLimit` - максимальное кол-во параллельных запросов
...
### `maxRequestDelayTimeMs` - максимальное время задержки между запросами
...
### `parallelLimitCorrectionPeriodMs` - период пересмотра параллельных запросов
